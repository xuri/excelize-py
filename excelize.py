...
from types_go import _RichTextRun,_DocProperties#修改部分
from types_py import RichTextRunsResult#修改部分
from ctypes import (
    byref,
    c_bool,
    c_char_p,
...
class File:
    ...
    def set_doc_props(self, props: DocProperties) -> Optional[Exception]:
        ...
        c_props = py_value_to_c(props, _DocProperties())
        if c_props is None:
    ...
    def get_cell_rich_text(
        ...
        Returns:
            Tuple[List[RichTextRun], Optional[Exception]]: 
                ...
                - Optional[Exception]: None if successful, Exception with error message if failed
        """
        lib.GetCellRichText.restype = types_go._RichTextRunsResult
        res = lib.GetCellRichText(
            self.file_index,
            sheet.encode(ENCODE),
            cell.encode(ENCODE)
        )
        Runs = c_value_to_py(res, RichTextRunsResult()).Runs
